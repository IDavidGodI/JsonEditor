...
const ActionCard = ({nameChanged ,closeFloating, name, addAction, setFloating, values}: ActionCardProps) => {
  return (
...
const PiecesSection = ({file,setFloating, closeFloating, loadedFile, floating, setLoadedFile}: PiecesSectionProps) => {
        ...
        <PreviewSection title="Actions" level={4} addForm={<NewActionForm afterSubmit={closeFloating} addAction={addAction} />} setFloating={setFloating}>
          {
            ...
            <div ref={cardsRef} className="flex gap-x-2 overflow-x-auto p-2 w-full ">
              {
                  actions.map(([name, values]) => <ActionCard key={name} nameChanged={nameChanged}  setFloating={setFloating} addAction={addAction} closeFloating={closeFloating} name={name} values={values}/>)
              }    
            </div>
          }
          </PreviewSection>
